...
if (!supabaseUrl || !supabaseKey) {
	...
	console.error('   This script requires the service role key for write operations.')
	process.exit(1)
}
...
const ORDER_FILES = ['EtsySoldOrderItems2025.csv', 'EtsySoldOrderItems2026.csv']
...
async function importReviews() {
	...
	for (const review of reviews) {
		...
		const { data: products } = await supabase.from('products').select('id, title').ilike('title', `%${searchTerm}%`).limit(1)
