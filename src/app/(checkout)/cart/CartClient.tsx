...
export function CartClient() {
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      // Only count a checkout after the server has priced the cart, persisted
      // the order, and received a valid Stripe destination.
      trackCheckoutStart(
        items,
        currency,
        promo.appliedCode ?? undefined,
        promo.totalAfterDiscountCents / 100,
      );
...
/**
 * Cart contents live in localStorage and cannot be rendered on the server.
 * Reserve the real two-column shape until Zustand hydrates instead of showing
 * an empty half-page that jumps when line items appear.
 */
function CartHydrationSkeleton() {
  return (
    <div aria-hidden className="animate-pulse">
      <div className="mb-8 space-y-2">
        <div className="h-3 w-24 rounded-full bg-[var(--muted)]" />
        <div className="h-10 w-52 rounded-2xl bg-[var(--muted)]" />
      </div>
      <div className="grid gap-6 lg:grid-cols-[1fr_24rem] lg:gap-10">
        <div className="space-y-3">
          {Array.from({ length: 2 }, (_, index) => (
            <div
              key={index}
              className="flex gap-4 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-3.5 sm:p-4"
            >
              <div className="h-24 w-24 shrink-0 rounded-2xl bg-[var(--muted)] sm:h-28 sm:w-28" />
              <div className="flex flex-1 flex-col gap-3 py-1">
                <div className="h-5 w-4/5 rounded bg-[var(--muted)]" />
                <div className="h-4 w-1/2 rounded bg-[var(--muted)]" />
                <div className="mt-auto h-9 w-28 rounded-full bg-[var(--muted)]" />
              </div>
            </div>
          ))}
        </div>
        <div className="h-[30rem] rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6">
          <div className="h-6 w-36 rounded bg-[var(--muted)]" />
          <div className="mt-5 h-16 rounded-2xl bg-[var(--muted)]" />
          <div className="mt-4 space-y-3">
            {Array.from({ length: 4 }, (_, index) => (
              <div key={index} className="h-5 rounded bg-[var(--muted)]" />
            ))}
          </div>
          <div className="mt-6 h-12 rounded-full bg-[var(--muted)]" />
        </div>
      </div>
    </div>
  );
