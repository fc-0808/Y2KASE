 ...
 *
 * Each run posts the next un-pinned real product photo(s) to Pinterest and
 * records them as pinned, so the whole catalog is distributed one Pin at a time
 * on a steady cadence — the consistency-first approach Pinterest's algorithm
 * rewards. Idempotent and safe to overlap: every image is claimed atomically, so
 * a given catalog image becomes exactly one Pin (see lib/social/auto-pin).
 *
