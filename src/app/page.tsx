              ...
              <Link key={idx} href={category.href} className="group block">
                <div className="card-y2k overflow-hidden">
                  <div className="relative aspect-4/5 overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 bg-linear-to-t from-pink-500/80 via-transparent to-transparent" />
                    
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      <span className="text-4xl mb-2 block">{category.emoji}</span>
                      <p className="text-white/80 text-sm font-medium mb-1">{category.subtitle}</p>
                      <h3 className="font-display text-white text-2xl font-bold mb-3">{category.title}</h3>
        ...
        <div className="container-y2k">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-12">
            <div className="text-center md:text-left mb-6 md:mb-0">
              ...
              <h2 className="font-display text-y2k-800 text-3xl md:text-4xl font-bold">Trending Now</h2>
              <p className="text-chrome-600 mt-2">Our most-loved phone cases</p>
            </div>
            <Link href="/products" className="inline-flex items-center justify-center md:justify-start gap-2 text-pink-500 font-semibold hover:text-pink-600 transition-colors">
              View All Cases
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
              </svg>
            ...
            {featuredProducts.map((product) => (
              <div key={product.id} className="group">
                  ...
                  <div className="relative aspect-3/4 overflow-hidden">
                    ...
                    {product.badge && (
                      <div className="absolute top-3 left-3">
                        <span className={`inline-block px-3 py-1.5 text-xs font-bold uppercase rounded-full ${product.badge === 'Bestseller' ? 'badge-hot' : product.badge === 'Sale' ? 'badge-sale' : 'badge-new'}`}>
                          {product.badge}
                        </span>
                      </div>
                    )}
                ...
                <div className="text-center">
                  <h3 className="font-display text-y2k-800 text-base md:text-lg font-semibold mb-2 group-hover:text-pink-500 transition-colors">{product.name}</h3>
                  <div className="flex items-center justify-center gap-2">
                    <span className="text-pink-500 font-bold text-lg">${product.price}</span>
                    {product.originalPrice && <span className="text-chrome-400 text-sm line-through">${product.originalPrice}</span>}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="container-y2k relative z-10">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-white/80 text-lg mb-8">Join thousands of happy customers who&apos;ve found their perfect case match.</p>
            <Link href="/products" className="btn bg-white text-pink-500 hover:bg-pink-50 text-lg px-8 py-4">Find Your Match ✨</Link>
          </div>
        </div>
      </section>
        ...
        <div className="container-y2k">
          <div className="py-20">
            <div className="max-w-lg mx-auto text-center">
              ...
              <h2 className="font-display text-y2k-800 text-3xl md:text-4xl font-bold mb-4">Join the Y2K Fam!</h2>
              <p className="text-chrome-600 mb-8">Get exclusive deals, early access to new drops, and cute content delivered to your inbox!</p>
              ...
              <p className="text-chrome-400 text-xs">No spam, just good vibes! Unsubscribe anytime.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-y2k-50">
        <div className="container-y2k">
          <div className="text-center mb-10">
            ...
            <h2 className="font-display text-y2k-800 text-2xl md:text-3xl font-bold mb-2">Follow Us @Y2KASE</h2>
            <p className="text-chrome-600">Tag us in your pics for a chance to be featured!</p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <a key={i} href="https://www.etsy.com/shop/Y2KASEstudio" target="_blank" rel="noopener noreferrer" className="relative aspect-square overflow-hidden rounded-xl group">
                <Image
                  src={`https://images.unsplash.com/photo-${['1601784551446-20c9e07cdbdb', '1592899677977-9c10ca588bbd', '1609692814858-f7cd2f0afa4f', '1556656793-08538906a9f8', '1585060544812-6b45742d762f', '1511707171634-5f897ff02aa9'][i - 1]}?w=300&h=300&fit=crop`}
        ...
        <div className="container-y2k">
          <div className="py-16">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
              <div>
                ...
                <div className="flex items-center gap-3">
                  {[
                    ...
                    <a key={social.name} href={social.name === 'Etsy' ? 'https://www.etsy.com/shop/Y2KASEstudio' : '#'} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-y2k-800 rounded-full flex items-center justify-center text-lg hover:bg-pink-500 transition-colors" aria-label={social.name}>
                      {social.icon}
                    </a>
                  ))}
                </div>
              ...
              <div>
                <h5 className="text-white font-display font-semibold mb-6">Shop 🛒</h5>
                <ul className="space-y-3">
                  {['All Cases', 'iPhone Cases', 'Samsung Cases', 'New Arrivals', 'Best Sellers', 'Sale'].map((item) => (
                    <li key={item}>
                      <Link href="/products" className="text-y2k-300 text-sm hover:text-pink-400 transition-colors">{item}</Link>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div>
                <h5 className="text-white font-display font-semibold mb-6">Help 💬</h5>
                <ul className="space-y-3">
                  {['Contact Us', 'Shipping Info', 'Returns & Exchanges', 'Size Guide', 'FAQ', 'Track Order'].map((item) => (
                    <li key={item}>
                      <Link href="/contact" className="text-y2k-300 text-sm hover:text-pink-400 transition-colors">{item}</Link>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div>
                <h5 className="text-white font-display font-semibold mb-6">Say Hi! 👋</h5>
                <div className="space-y-4 text-y2k-300 text-sm">
                  <p className="flex items-center gap-2">
                    <span>📧</span>
                    <a href="mailto:hello@y2kase.com" className="hover:text-pink-400 transition-colors">hello@y2kase.com</a>
                  </p>
          ...
          <div className="border-t border-y2k-800">
            <div className="py-6 flex flex-col md:flex-row items-center justify-between gap-4">
              <p className="text-y2k-400 text-sm">© {currentYear} Y2KASE. Made with 💖</p>
              <div className="flex items-center gap-6">
                {['Privacy Policy', 'Terms of Service', 'Shipping Policy'].map((item) => (
                  <Link key={item} href="#" className="text-y2k-400 text-sm hover:text-pink-400 transition-colors">{item}</Link>
                ))}
              </div>
            </div>
          </div>
