...
export const emailSubscribers = pgTable(
  "email_subscribers",
  {
    ...
    /** active | unsubscribed */
    status: text("status").notNull().default("active"),
    /** Versioned disclosure plus privacy-minimized evidence of explicit opt-in. */
    consentVersion: text("consent_version"),
    consentRecordedAt: timestamp("consent_recorded_at", { withTimezone: true }),
    consentIpHash: text("consent_ip_hash"),
    consentUserAgent: text("consent_user_agent"),
    consentLocale: text("consent_locale"),
    consentCountry: text("consent_country"),
    subscribedAt: timestamp("subscribed_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
    resubscribedAt: timestamp("resubscribed_at", { withTimezone: true }),
    unsubscribedAt: timestamp("unsubscribed_at", { withTimezone: true }),
    /** customer_one_click | admin | provider_opt_out | bounce | complaint | suppressed */
  ...
  (t) => [
    ...
    check(
      ...
      sql`${t.status} in ('active', 'unsubscribed')`,
    ),
    check(
      "email_subscribers_country_check",
      sql`${t.consentCountry} is null or char_length(${t.consentCountry}) = 2`,
    ),
    check(
      "email_subscribers_unsubscribe_reason_check",
      sql`${t.unsubscribeReason} is null or ${t.unsubscribeReason} in ('customer_one_click', 'admin', 'provider_opt_out', 'bounce', 'complaint', 'suppressed')`,
    ),
  ],
);
...
/**
 * marketing_campaigns — durable control plane for subscriber broadcasts.
 *
 * Resend remains the delivery provider, but this table owns the human-review
 * workflow and idempotency boundary. In particular, persisting the provider's
 * broadcast id before sending lets a retry inspect/reuse the same broadcast
 * instead of creating a duplicate send.
 */
export const marketingCampaigns = pgTable(
  "marketing_campaigns",
  {
    /** UUID generated by the composer and reused across every retry. */
    id: text("id").primaryKey(),
    createdBy: text("created_by").references(() => users.id, {
      onDelete: "set null",
    }),
    name: text("name").notNull(),
    campaignType: text("campaign_type").notNull().default("announcement"),
    subject: text("subject").notNull(),
    ...
    /** draft | preparing | queued | scheduled | sent | cancelled | failed */
    status: text("status").notNull().default("draft"),
    /** Hash of every field that affects the rendered email. */
    contentHash: text("content_hash").notNull(),
    /** Must equal content_hash before a production launch is allowed. */
    testedContentHash: text("tested_content_hash"),
    ...
    preparedAudienceHash: text("prepared_audience_hash"),
    preparedRecipientCount: integer("prepared_recipient_count"),
    preparedAt: timestamp("prepared_at", { withTimezone: true }),
    launchedBy: text("launched_by").references(() => users.id, {
      onDelete: "set null",
    }),
    launchAttemptId: text("launch_attempt_id"),
  ...
  (t) => [
    uniqueIndex("marketing_campaigns_resend_broadcast_idx").on(
      t.resendBroadcastId,
    ),
    index("marketing_campaigns_status_idx").on(t.status),
    index("marketing_campaigns_created_idx").on(t.createdAt),
    check(
      "marketing_campaigns_status_check",
      sql`${t.status} in ('draft', 'preparing', 'queued', 'scheduled', 'sent', 'cancelled', 'failed')`,
    ),
    check(
      "marketing_campaigns_recipient_count_check",
      sql`(${t.recipientCount} is null or ${t.recipientCount} >= 0) and (${t.preparedRecipientCount} is null or ${t.preparedRecipientCount} >= 0)`,
    ),
  ],
);
...
export const resendWebhookEvents = pgTable(
  ...
  (t) => [index("resend_webhook_events_processed_idx").on(t.processedAt)],
);
